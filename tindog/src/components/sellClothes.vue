        ...
        methods:
        {
            ...
            postFeed()
            {
                ...
                let newPostkey = firebase.database().ref(`Users/${uid}/AccessoriesPost/Clothes`).push({
                    ...
                    let key = data.key
                    firebase.storage().ref(`Images/Accessoriesfeed/Clothes/${key}`).put(imageUpload)
            ...
            liveAddingPost()
            {
                ...
                firebase.database().ref('Accessories/Clothes').on('value',snap => {
                    ...
                    snap.forEach(childSnap => {
                        ...
                        let promise = firebase.storage().ref(`Images/Accessoriesfeed/Clothes/${childSnap.key}`).getDownloadURL().then(url => {
                            return { 
                                key: childSnap.key,
                                image: url,
                                uid: valId,
